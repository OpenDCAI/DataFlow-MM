...
class CTCForcedAlignmentSampleEvaluator(OperatorABC):
    ...
    def get_desc(lang: str = "zh"):
        if lang == "zh":
            return (
                "CTCForcedAlignmentSampleEvaluator（CTC 强制对齐评估算子）\n"
                "------------------------------------------------------------\n"
                "功能简介：\n"
                "该算子使用 CTC forced alignment 模型将音频与其转录文本进行对齐，输出逐词/片段级别的时间戳与对齐得分，"
                "用于数据质量评估、标注辅助或对齐诊断。支持串行与多进程并行两种运行方式：\n"
                "- 串行模式（num_workers<=1）：主进程加载一次模型并逐条处理。\n"
                "- 并行模式（num_workers>1）：使用 spawn 启动多个子进程，每个子进程各自加载一份模型，主进程负责切分数据并分发任务。\n\n"
                "一、__init__ 初始化接口\n"
                "def __init__(\n"
                "    self,\n"
                "    model_path: str = \"MahmoudAshraf/mms-300m-1130-forced-aligner\",\n"
                ...
                ")\n\n"
                "参数说明：\n"
                "- model_path：对齐模型路径（HuggingFace repo id 或本地路径）。\n"
                "- device：推理设备。\n"
                "  * 串行模式：\"cpu\" / \"cuda\" / \"cuda:0\" 等单个字符串；\n"
                "  * 并行模式：可传字符串列表，如 [\"cuda:0\", \"cuda:1\"]，子进程按 worker rank 轮询分配设备。\n"
                "- num_workers：工作进程数；>1 时启用多进程并行并在每个子进程中加载模型。\n"
                "- sampling_rate：读取与重采样音频的采样率（Hz）。\n"
                "- language：文本语言代码（如 \"en\"、\"zh\"），用于文本处理/罗马化时选择 ISO 639-3 代码。\n"
                "- micro_batch_size：生成 emissions 时的微批大小，用于控制显存与吞吐。\n"
                "- chinese_to_pinyin：若为 True，会在对齐前将中文转为拼音（空格分隔）。\n"
                "- romanize：若为 True，会使用 uroman 进行罗马化（适用于非拉丁文字）。\n\n"
                "输出（初始化行为）：\n"
                "- 串行模式：在主进程创建并持有 Aligner 实例。\n"
                "- 并行模式：创建 multiprocessing pool，并在每个子进程中初始化 Aligner（主进程不持有模型实例）。\n\n"
                "二、run 运行接口\n"
                "def run(\n"
                "    self,\n"
                ...
                "- input_audio_key：音频路径列名。每行可为字符串或单元素列表（[path]）。支持本地路径与 http/https。\n"
                "- input_conversation_key：文本列名。\n"
                "  * 若单元格是字符串：直接作为转录文本；\n"
                "  * 若单元格是 list 且 list[0] 为 dict 且包含 'value'：使用 conversation[0]['value']。\n"
                "- output_answer_key：输出列名。\n\n"
                "输出说明：\n"
                "run 会将对齐结果写回 DataFrame 的 output_answer_key 列，并通过 storage.write 持久化。\n"
                "每行输出为一个 dict：\n"
                "{\n"
                "  'alignment': List[Dict],  # 逐词/片段对齐结果，每个元素包含 start/end/text/score\n"
                "  'error': Optional[str],  # 成功为 None，失败为错误信息\n"
                "}\n"
                "其中 alignment 的 score 为指数化后的值（exp(score)），可近似理解为置信度。\n"
            )
        else:
            return (
                "CTCForcedAlignmentSampleEvaluator (CTC Forced Alignment Evaluator)\n"
                "---------------------------------------------------------------\n"
                "Overview:\n"
                "This operator performs CTC-based forced alignment between speech audio and its transcript, "
                "producing word/segment-level timestamps and alignment scores. It is useful for data QA, "
                "annotation assistance, and alignment diagnostics. It supports both serial and multi-process execution:\n"
                "- Serial (num_workers<=1): the model is loaded once in the main process and samples are processed sequentially.\n"
                "- Parallel (num_workers>1): child processes are spawned (spawn context); each worker loads its own model instance, "
                "while the main process only splits data and dispatches jobs.\n\n"
                ...
                ")\n\n"
                "Parameters:\n"
                "- model_path: pretrained aligner model path (HF repo id or local path).\n"
                "- device: inference device(s).\n"
                "  * Serial: a single string like \"cpu\", \"cuda\", \"cuda:0\".\n"
                "  * Parallel: a list like [\"cuda:0\", \"cuda:1\"], assigned to workers in a round-robin manner by worker rank.\n"
                "- num_workers: number of worker processes; >1 enables multiprocessing with one model per worker.\n"
                "- sampling_rate: audio loading/resampling rate in Hz.\n"
                "- language: language code (e.g., \"en\", \"zh\"), used to derive ISO 639-3 code for text processing/romanization.\n"
                "- micro_batch_size: micro-batch size for emission generation (trade-off between memory and throughput).\n"
                "- chinese_to_pinyin: if True, converts Chinese text to space-separated pinyin before alignment.\n"
                "- romanize: if True, romanizes text via uroman (useful for non-Latin scripts).\n\n"
                "Initialization behavior:\n"
                "- Serial: creates and holds a single Aligner instance in the main process.\n"
                "- Parallel: creates a multiprocessing pool; each worker initializes its own Aligner instance (main process does not hold the model).\n\n"
                "2) run interface\n"
                "def run(\n"
                "    self,\n"
                "    storage: DataFlowStorage,\n"
                ...
                "- input_audio_key: column name for audio paths. Each cell can be a string or a single-element list. "
                "Local paths and http/https URLs are supported.\n"
                "- input_conversation_key: column name for transcripts.\n"
                "  * If the cell is a string: it is treated as the transcript.\n"
                "  * If the cell is a list and list[0] is a dict containing 'value': conversation[0]['value'] is used.\n"
                "- output_answer_key: name of the output column.\n\n"
                "Outputs:\n"
                "The operator writes alignment results back to the DataFrame as output_answer_key and persists it via storage.write.\n"
                "Each row stores a dict:\n"
                "{\n"
                "  'alignment': List[Dict],  # word/segment alignments with start/end/text/score\n"
                "  'error': Optional[str],  # None on success, otherwise an error message\n"
                "}\n"
                "The returned score is exponentiated (exp(score)), which can be interpreted as a confidence-like value.\n"
            )
