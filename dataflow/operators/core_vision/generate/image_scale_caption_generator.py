    ...
    def get_desc(lang: str = "zh") -> str:
        if str(lang).lower().startswith("zh"):
            return (
                ...
                "  • Serving 约定（VLMServingABC.generate_from_input_messages）：\n"
                "    - 带图对话需在 human 的 value 中包含 '<image>' 占位，并通过 image_list=[[img_path]] 传入图像。\n"
                "    - 纯文本调用仅传 human value 文本即可。\n"
                "\n"
            ...
            return (
                ...
                "  • Serving contract (VLMServingABC.generate_from_input_messages):\n"
                "    - For vision inputs, put '<image>' in the human message value and pass the image via image_list=[[img_path]].\n"
                "    - For text-only, send a single human message.\n"
                "\n"
