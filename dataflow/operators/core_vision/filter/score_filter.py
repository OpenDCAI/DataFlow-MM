...
class ScoreFilter(OperatorABC):
    ...
    def __init__(
        ...
        Args:
            score_key: 要过滤的分数列名（如 "reward", "accuracy" 等）
        ...
        self.output_select_key = output_select_key
    ...
    def run(
        ...
        Returns:
            str: 输出列名
        """
        self.logger.info(f"Starting ScoreFilter on column '{self.score_key}'...")
        ...
        # 检查分数列是否存在
        if self.score_key not in df.columns:
            raise ValueError(f"Score column '{self.score_key}' not found in dataframe")
        
        # 应用过滤条件
        select_mask = pd.Series([True] * len(df), index=df.index)
        
        if self.min_score is not None:
            select_mask &= df[self.score_key] >= self.min_score
        
        if self.max_score is not None:
            select_mask &= df[self.score_key] <= self.max_score
        
        # 添加筛选标记列
        df[self.output_select_key] = select_mask
